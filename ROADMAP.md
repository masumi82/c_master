...
## 別件（2〜3日）: 上流工程
要件定義書と基本設計書を書く練習。C の1週間とは独立に進められますが、題材の UART コマンド仕様を ex04 の振り分け方式に合わせるので、`phase1_lang/ex04_cmd_table` の README だけ先に読んでください。進め方は `upstream/README.md`。
- [ ] 1日目: ヒアリング（Claude が顧客役）→ 要件定義書
- [ ] 2日目: 基本設計書（構成図・コマンド仕様・状態遷移）
- [ ] 3日目: トレーサビリティ表 → レビュー → 3文書の修正
